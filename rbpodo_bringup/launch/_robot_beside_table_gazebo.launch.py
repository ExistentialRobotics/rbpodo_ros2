...
def build_robot_description(this_robot_prefix="", this_robot_namespace="", add_gripper = False):
    ...
    robot_description = {
        'robot_description': get_xacro_command(
            xacro_file=PathJoinSubstitution([FindPackageShare('rbpodo_description'), 'robots', 'rb10_1300e.urdf.xacro']), 
            mappings = {
                "robot_ip": "10.0.2.7",
                "cb_simulation": "true",
...
def get_per_robot_stack(robot_idx, load_controller):
    ...
    # with open('rb10_desc.txt', 'w') as file:
    #     # Write content to the file
    #     for key, value in robot_description.items():
    #         params = vars(value)
